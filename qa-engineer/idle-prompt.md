...
1. **Unreviewed PRs (top priority):**
   ```
   tea pr list --repo molecule-ai/molecule-core --state open --json number,title,reviews --limit 20 | python3 -c "
   import json,sys
...
2. If no unreviewed PRs, check for issues labeled `needs-work`:
   `tea issue list --repo molecule-ai/molecule-core --label needs-work --state open --limit 5`
